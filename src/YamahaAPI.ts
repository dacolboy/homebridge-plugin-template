...
export class YamahaAPI {
  ...
  public async getPresetInfo(host: string): Promise<PresetInfoResponse> {
    ...
    const presetInfos = await this.httpRequest(url) as PresetInfoResponse;
    for (let i = 0; i < presetInfos.preset_info.length; i++) {
      ...
      presetInfos.preset_info[i].hidden = 0;
    ...
    presetInfos.preset_info = presetInfos.preset_info.filter((presetInfo) => 
      (presetInfo.input === 'server' || presetInfo.input === 'net_radio') && presetInfo.text !== '');
    return presetInfos;
  }
  ...
  public async setPower(host: string, power: boolean): Promise<Response> {
    const parameter = power ? 'on' : 'standby';
    let ret: Promise<unknown> = Promise.resolve();
    if (power) {
      const url1 = 'http://' + host + '/YamahaExtendedControl/v1/' + this.zone + '/setInput?input=net_radio';
      ret = this.httpRequest(url1);
    }
    const url = 'http://' + host + '/YamahaExtendedControl/v1/' + this.zone + '/setPower?power=' + parameter;
    return ret.then(() => this.httpRequest(url).then(result => result as Response));
  }
  ...
  public async setMute(host: string, enable: boolean): Promise<Response> {
    const url = 'http://' + host + '/YamahaExtendedControl/v1/' + this.zone + '/setMute?enable=' + enable;
    return this.httpRequest(url).then(result => result as Response);
  }
  ...
  public async setEnhancer(host: string, enable: boolean): Promise<Response> {
    const url = 'http://' + host + '/YamahaExtendedControl/v1/' + this.zone + '/setEnhancer?enable=' + enable;
    return this.httpRequest(url).then(result => result as Response);
  }
}
